<template>
    <div class="alert alert-light alert-dismissible fade show" role="alert">
        {{amenity.name}}
        <button @click="removeAmenityFromRoom()" type="button" class="close" :title="`Remove ${amenity.name}`">
...
export default defineComponent({
    ...
    setup(props, {emit}){
        const {roomId, amenity} = toRefs(props);
        const store = useStore();
        const {removeRoomAmenity, setSnackbar,} = useProperty(store);
        const removeAmenityFromRoom = async () => {
            try {
                ...
                let response = await removeRoomAmenity({roomId: roomId.value, amenityId: amenity.value.id});
